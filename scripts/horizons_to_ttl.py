...
def horizons_elements(command: str, center: str, out_units: str) -> dict:
    ...
    if "$$SOE" not in result or "$$EOE" not in result:
        raise RuntimeError(
            f"Horizons no devolvió una tabla válida para COMMAND={command}, CENTER={center}"
        )
    ...
    return {
        "fechaEpoch": start,
        "epochJulianDate": to_float(row[0]),
...
def add_orbit_ttl(lines, orbit_uri, orbit, unit: str):
    """
    unit = 'AU' para planetas, planetas enanos y asteroides.
    unit = 'KM' para satélites.
    """
    ...
    if unit == "AU":
        props = [
            ("fechaEpoch", "fechaEpoch", "date"),
            ("epochJulianDate", "epochJulianDate", "decimal"),
            ...
            ("argumentoPerihelioDeg", "argumentoPeriapsisDeg", "decimal"),
...
def add_heliocentric_bodies(lines, title, bodies, rdf_class):
    """
    Genera cuerpos que orbitan el Sol:
    - planetas
    - planetas enanos
    - asteroides
    """
    ...
    for slug, info in bodies.items():
        print(f"Consultando {title.lower()}: {info['nombre']}...")
...
def build_ttl() -> str:
    lines = [
        ...
        "",
        "# ── GENERADO DESDE NASA/JPL HORIZONS ───────────────────────",
        "",
        "sol:sol",
        "    a sol:Estrella ;",
        '    sol:nombre "Sol" ;',
    ...
    for slug, info in SATELLITES.items():
        ...
        lines.extend([
            ...
            f"    sol:tieneOrbita {orbit_uri} .",
            "",
            f"{parent_uri} sol:tieneSatelite sol:{slug} .",
            "",
        ])
...
if __name__ == "__main__":
    ...
    print("Archivo generado: data_horizons.ttl")